...
class Review {
	...
	private function __construct() {
        add_action( 'admin_init', [ __CLASS__, 'cptint_check_installation_time' ] );
        add_action( 'admin_init', [ __CLASS__, 'cptint_spare_me' ], 5 );
	}
	...
	public static function cptint_check_installation_time() {
        ...
        add_action( 'admin_notices', [ __CLASS__, 'cptint_display_admin_notice' ] );
    ...
    public static function cptint_spare_me() {
        ...
        if ( ! isset( $_REQUEST['_wpnonce'] ) || ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'cptint_notice_nonce' ) ) {
            return;
        }
	...
	protected static function cptint_current_admin_url() {
		...
		return remove_query_arg( [ '_wpnonce', '_wc_notice_nonce', 'wc_db_update', 'wc_db_update_nonce', 'wc-hide-notice', 'cptint_spare_me', 'cptint_remind_me', 'cptint_rated' ], admin_url( $uri ) );
	}
	...
	public static function cptint_display_admin_notice() {
		...
		if ( ! in_array( $pagenow, $exclude ) ) {
			...
			$args = [ '_wpnonce' => wp_create_nonce( 'cptint_notice_nonce' ) ];

			$dont_disturb = add_query_arg( $args + [ 'cptint_spare_me' => '1' ], self::cptint_current_admin_url() );
			$remind_me    = add_query_arg( $args + [ 'cptint_remind_me' => '1' ], self::cptint_current_admin_url() );
			$rated        = add_query_arg( $args + [ 'cptint_rated' => '1' ], self::cptint_current_admin_url() );
